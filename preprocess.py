...
class Preprocessor(object):
    def __init__(self, version, para_num=-1):
        ...
        if version == "2.0":
            data_name = "paraphrased_data.json"
            ...
            raise Exception("Unsupported version of data")
    ...
    def preprocess(self):
        ...
        for fn, raw_dial in tqdm(list(self.data.items())):
            ...
            para_dial = {}
            para_single_turn = {}
            for p_num in range(self.para_num):
                para_single_turn[p_num] = {}
                para_dial[p_num] = {'goal': compressed_goal, 'log': []}
            # ----
            ...
            for turn_num, dial_turn in enumerate(raw_dial['log']):
                ...
                if self.para_num > 0:
                    for p_num in range(self.para_num):
                        dial_turn['text_' + str(p_num)] = ' '.join([t.text for t in self.nlp(dial_turn['text_' + str(p_num)])])
                        dial_turn["text_" + str(p_num)] = ' '.join(dial_turn["text_" + str(p_num)].replace(".", " . ").split())
                ...
                if not dial_state:  # user
                    ...
                    # -----
                    if self.para_num > 0 and (fn not in self.dev_list + self.test_list):
                        for p_num in range(self.para_num):
                            u = ' '.join(clean_text(dial_turn['text_' + str(p_num)]).split())
                            u_delex = self.delex_by_valdict(dial_turn['text_' + str(p_num)])
                            para_single_turn[p_num]['user'] = u
                            para_single_turn[p_num]['user_delex'] = u_delex
                    # ----
                    ...
                    if self.para_num > 0 and (fn not in self.dev_list + self.test_list):
                        for p_num in range(self.para_num):
                            s_delex = self.delex_by_valdict(dial_turn['text_' + str(p_num)])
                            para_single_turn[p_num]['resp'] = s_delex
                            para_single_turn[p_num]['nodelx_resp'] = ' '.join(clean_text(dial_turn['text_' + str(p_num)]).split())
                    # ---
                    ...
                    if self.para_num > 0 and (fn not in self.dev_list + self.test_list):
                        for p_num in range(self.para_num):
                            para_single_turn[p_num]['pointer'] = ','.join([str(d) for d in dbvec + bkvec])
                            para_single_turn[p_num]['match'] = str(match)
                            para_single_turn[p_num]['constraint'] = ' '.join(constraints)
                            para_single_turn[p_num]['cons_delex'] = ' '.join(cons_delex)
                            para_single_turn[p_num]['sys_act'] = ' '.join(sys_act)
                            para_single_turn[p_num]['turn_num'] = len(dial['log'])
                            para_single_turn[p_num]['turn_domain'] = ' '.join(['[' + d + ']' for d in turn_domain])
                    ...
                    if 'user' in single_turn:
                        ...
                        if self.para_num > 0 and (fn not in self.dev_list + self.test_list):
                            for p_num in range(self.para_num):
                                para_dial[p_num]['log'].append(para_single_turn[p_num])

                                for t in para_single_turn[p_num]['user_delex'].split():
                                    if '[' in t and ']' in t and not t.startswith('[') and not t.endswith(']'):
                                        para_single_turn[p_num]['user_delex'].replace(t, t[t.index('['): t.index(']') + 1])
                        # ----
            ...
            if fn in self.dev_list:
                ...
                train_data[fn] = dial
                for p_num in range(self.para_num):
                    train_data[str(p_num) + '_' + fn] = para_dial[p_num]
...
if __name__ == "__main__":
    ...
    parser.add_argument("-version", type=str, default="2.1", choices=["2.0", "2.1", "2.4"])
    parser.add_argument("-para_num", type=int, default=5)

    args = parser.parse_args()

    preprocessor = Preprocessor(args.version, para_num=args.para_num)
