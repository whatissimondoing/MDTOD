...
class MultiWozEvaluator(object):
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, real_requestables, counts, soft_acc=False):
    ...
    def e2e_eval(self, data, eval_dial_list=None):
        ...
        success, match, req_offer_counts, dial_num = self.context_to_response_eval(data, eval_dial_list=eval_dial_list)

        return bleu, success, match


class IncarEvaluator(object):
    def __init__(self, reader, eval_data_type="test"):
        self.reader = reader
        self.all_domains = definitions.ALL_DOMAINS
        ...
        for d, s_list in definitions.INFORMABLE_SLOTS.items():
            for s in s_list:
                self.all_info_slot.append(d + '-' + s)
                self.informable_slots.append(s)
    ...
    def _context_to_response_evl(self, dials, domain=None, file_list=None):

        match = self.match_metric(dials)

        success = self.request_metric(dials)

        bleu = self.bleu_metric(dials)

        return bleu, 100 * success, 100 * match

    def _normalize_constraint(self, constraint, ignore_dontcare=True, intersection=True):
        """
        ...
        normalized = {}
        entities_flat = ['event', 'time', 'date', 'party', 'room', 'agenda', 'location', 'weekly_time', 'temperature', 'weather_attribute',
                         'traffic_info', 'poi_type', 'poi', 'distance']
        for s in self.informable_slots:  # entities_flat:#self.informable_slots:
            normalized[s] = ''
        for _, c in constraint.items():
            for s, v in c.items():
    ...
    def request_metric(self, data):
        ...
        for dial_id in dials:
            ...
            dial = dials[dial_id]
            for turn_num, turn in enumerate(dial):
                resp_gen_token = turn['resp_gen'].split()
                resp_token = turn['resp'].split()
    ...
    def e2e_eval(self, data, eval_dial_list=None):
        return self._context_to_response_evl(data)
...
if __name__ == "__main__":
    ...
    if cfg.task == "e2e":
        bleu, success, match = evaluator.e2e_eval(data, eval_dial_list=eval_dial_list)
        ...
        joint_goal, f1, accuracy, _, _ = evaluator.dialog_state_tracking_eval(data, eval_dial_list=eval_dial_list)
